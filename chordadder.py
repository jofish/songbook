# -*- coding: utf-8 -*-
import xml.dom.minidom, string, re, codecs, sys, os
...
files_a = [os.path.splitext(f)[0].lower() for f in os.listdir("./diagrams/guitar")]
files_b = [os.path.splitext(f)[0].lower() for f in os.listdir("./diagrams/guitar-truechord")]
allknownchords = files_a + files_b
#print(allknownchords)

modwords = ["sus","min","maj","dim","aug","add"]
...
#readInSongs(xml.dom.minidom.parse(xmlfile, encoding='utf-8'))
...
for song in main.getElementsByTagName("song"):
	...
	for chord in re.findall(r"\[.*?\]",lyrics):
	...
	for line in lyrics.split('\n'):
		...
		#if float(len(line.replace(" ",'')))/float(len(line))<0.25:#if the line is more than half whitespace
		#this misses lines consisting of just chords.
		#ok now this bit is filthy but it'll do for now. really we should just look for filenames. anyhoo
		for chord in line.replace('\\',' ').split(): #fix all the slashes. this is a bad hack and we will fix in the future
			...
			for punctuationmark in string.punctuation.replace("#","").replace("/"," ").replace("+"," "): 
				chord=chord.replace(punctuationmark,"")  #let's get rid of punctuation except for those sharps and replaced slashes and pluses
			if len(chord)==0:continue
			for filler in nonchordwords:
				if filler in chord.lower():badchord=True 
			if chord[0] not in "ABCDEFG": badchord = True
			if (chord[-1] not in "0123456789mABCDEFGb#"): badchord = True
			if len(chord)>6: badchord=True
			if chord.split('/')[0].lower() not in allknownchords: badchord = True
			if badchord: #last chance
				for modword in modwords: 
					if modword in chord:
						if any(char.isdigit() for char in chord): badchord = False #if it's got a number, it's probably a chord not a word
						if """#""" in chord: badchord = False #if it's got a sharp, it's probably a chord not a word
						if """/""" in chord: badchord = False #if it's got a split, it's probably a chord not a word
						if """+""" in chord: badchord = False #if it's got a plus, it's probably a chord not a word
						if len(chord.split(modword)[0].upper())==1 and chord.split(modword)[0].upper() in "ABCDEFG": badchord = False
						if "aught" in chord.lower() or "daddy" in chord.lower() or "baddest" in chord.lower(): badchord = True
							#ugly but it works.
			if not badchord: countofchords+=1
...
with open(outfile, 'w', encoding='utf-8') as f:
	main.writexml(f,encoding='utf-8')
